...
export const AddCategoryForm = ({  setIfAddCategory, close }) => {
  ...
  return <div className="w-[494px] h-fit grid grid-rows-[1fr_3fr] bg-white rounded-2xl overflow-hidden">
  ...
  <form className="w-[494px] min-h-[236px] h-fit p-6 flex flex-col justify-around gap-2" action={onSumbit}>
    <div className="w-full min-h-1/2 h-fit flex flex-col gap-2">
      ...
      >
        <SelectedIcon selectedIcon={selectedIcon} selectedColor={selectedColor} />
        {categoriesOpen ? <UpOutlined /> : <DownOutlined />}
      </div>
      {categoriesOpen && <Categories icons={icons} colors={colors} setSelectedColor={setSelectedColor} setSelectedIcon={setSelectedIcon}/>}
      <input
        type="text"
        placeholder="Name"
        id="name"
